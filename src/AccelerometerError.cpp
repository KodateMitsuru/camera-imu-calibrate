...
AccelerometerErrorEccentric::AccelerometerErrorEccentric(
    ...
    const aslam::backend::EuclideanExpression& bias,
    const aslam::backend::EuclideanExpression& g_w)
    : _Ix(aslam::backend::MatrixBasic(Eigen::Vector3d(1., 0., 0.).asDiagonal(),
                                      Eigen::Matrix3i::Zero())),
      _Iy(aslam::backend::MatrixBasic(Eigen::Vector3d(0., 1., 0.).asDiagonal(),
                                      Eigen::Matrix3i::Zero())),
      _Iz(aslam::backend::MatrixBasic(Eigen::Vector3d(0., 0., 1.).asDiagonal(),
                                      Eigen::Matrix3i::Zero())) {
  _measurement = measurement;
  setInvR(invR);
  _predictedMeasurement =
      M * (C_i_b *
           (C_b_w * (acceleration_w - g_w) +
            _Ix.toExpression() *
                (angularAcceleration_b.cross(rx_b) +
                 angularVelocity_b.cross(angularVelocity_b.cross(rx_b))) +
            _Iy.toExpression() *
                (angularAcceleration_b.cross(ry_b) +
                 angularVelocity_b.cross(angularVelocity_b.cross(ry_b))) +
            _Iz.toExpression() *
                (angularAcceleration_b.cross(rz_b) +
