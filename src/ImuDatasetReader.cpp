...
bool ImuDatasetReaderIterator::operator!=(const ImuDatasetReaderIterator& other) const {
...
ImuDatasetReader::ImuDatasetReader(
    ...
    if (measurements_.empty()) {
        throw std::runtime_error("No valid IMU measurements found in CSV: " + csvFile);
    }
    ...
    std::cout << "ImuDatasetReader: Loaded " << indices_.size() 
              << " IMU measurements from " << csvFile << std::endl;
}
...
bool ImuDatasetReader::detectCSVFormat(const std::vector<std::string>& header) {
    ...
    for (size_t i = 0; i < header.size(); ++i) {
        ...
        if (col.find("time") != std::string::npos || col == "t") {
            colTimestamp_ = i;
            hasHeader = true;
        } else if (col.find("omega_x") != std::string::npos || col.find("gyro_x") != std::string::npos || 
                   col.find("wx") != std::string::npos || col.find("gx") != std::string::npos) {
            colOmegaX_ = i;
            hasHeader = true;
        } else if (col.find("omega_y") != std::string::npos || col.find("gyro_y") != std::string::npos ||
                   col.find("wy") != std::string::npos || col.find("gy") != std::string::npos) {
            colOmegaY_ = i;
            hasHeader = true;
        } else if (col.find("omega_z") != std::string::npos || col.find("gyro_z") != std::string::npos ||
                   col.find("wz") != std::string::npos || col.find("gz") != std::string::npos) {
            colOmegaZ_ = i;
            hasHeader = true;
        } else if (col.find("alpha_x") != std::string::npos || col.find("accel_x") != std::string::npos ||
                   col.find("ax") != std::string::npos) {
            colAlphaX_ = i;
            hasHeader = true;
        } else if (col.find("alpha_y") != std::string::npos || col.find("accel_y") != std::string::npos ||
                   col.find("ay") != std::string::npos) {
            colAlphaY_ = i;
            hasHeader = true;
        } else if (col.find("alpha_z") != std::string::npos || col.find("accel_z") != std::string::npos ||
...
void ImuDatasetReader::loadFromCSV() {
    ...
    try {
        ...
        if (file.is_open() && std::getline(file, firstLine)) {
            ...
            size_t maxColIdx = std::max({colTimestamp_, colOmegaX_, colOmegaY_, colOmegaZ_,
                                         colAlphaX_, colAlphaY_, colAlphaZ_});
            
            if (numCols <= maxColIdx) {
                throw std::runtime_error("CSV file does not have enough columns. Expected at least " + 
            ...
            for (size_t i = 0; i < numRows; ++i) {
                try {
                    ...
                    std::cerr << "Warning: Failed to parse row " << i << ": " << e.what() << std::endl;
                    continue;
                }
            }
...
void ImuDatasetReader::truncateIndicesFromTime(const std::pair<double, double>& from_to) {
    if (indices_.empty()) {
        return;
    }
    ...
    double baglength = *std::max_element(timestamps.begin(), timestamps.end()) - bagstart;
    
    std::cout << "IMU dataset start: " << bagstart << " s" << std::endl;
    std::cout << "IMU dataset length: " << baglength << " s" << std::endl;
    ...
    if (from_to.second > baglength) {
        std::cerr << "Warning: End time of " << from_to.second 
                  << " s is greater than the total length of " << baglength << " s" << std::endl;
    }
    ...
    if (removed > 0) {
        std::cout << "ImuDatasetReader: Truncated " << removed << " / " 
                  << indices_.size() << " messages (time-based)" << std::endl;
    }
