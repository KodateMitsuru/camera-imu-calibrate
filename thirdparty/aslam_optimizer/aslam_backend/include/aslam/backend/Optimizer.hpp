 ...
 * A Methodology for Estimation of Physical Parameters, Systems Analysis
 * Modelling Simulation, 43:7, 925-943
 *
 * Erik Etien, Damien Halbert, and Thierry Poinot
 * Improved Jiles�Atherton Model for Least Square Identification Using
 * Sensitivity Function Normalization IEEE TRANSACTIONS ON MAGNETICS, VOL. 44,
 * NO. 7, JULY 2008
 *
 */
class Optimizer {
 public:
  ...
  void setProblem(std::shared_ptr<OptimizationProblemBase> problem);

  /// \brief Set up to work on the optimization problem.
  void setProblem(OptimizationProblemBase* problem, bool optimizerOwnsProblem) {
    setProblem(optimizerOwnsProblem
                   ? std::shared_ptr<OptimizationProblemBase>(problem)
                   : std::shared_ptr<OptimizationProblemBase>(
                         problem, sm::null_deleter()));
  }
  ...
  /// \brief compute only the covariance blocks associated with the block
  /// indices passed as an argument
  void computeCovarianceBlocks(
      const std::vector<std::pair<int, int> >& blockIndices);

  /// \brief get a particular covariance block. If the block has not been
  /// computed, this will return NULL.
  const Eigen::MatrixXd* getCovarianceBlock(int blockRow, int blockCol) const;

  /// \brief get the covariance matrix as a sparse matrix. This matrix will only
  /// be filled in by what was computed above.
  ...
  /// \brief Do a bunch of checks to see if the problem is well-defined. This
  /// includes checking that every error term is
  ///        hooked up to design variables and running finite differences on
  ///        error terms where this is possible.
 ...
 private:
  ...
  /// \brief Set the initial lambda by looking at the entries of the Hessian
  /// matrix.
  ...
  std::shared_ptr<LinearSolver> _fallbackSolver;

  /// \brief The current optimization problem.
  std::shared_ptr<OptimizationProblemBase> _problem;

  /// \brief all design variables...first the non-marginalized ones (the dense
  /// ones), then the marginalized ones.
  ...
  /// \brief an index into the _designVariables member that tells where the
  /// first marginalized design variable lives.
