...
struct Optimizer2Options {
  ...
  /// \brief stop when steps cause changes in the objective function below this
  /// threshold.
  double convergenceDeltaJ;

  /// \brief stop when the maximum change in and component of a design variable
  /// drops below this threshold
  double convergenceDeltaX;

  /// \brief stop if we reach this number of iterations without hitting any of
  /// the above stopping criteria.
  int maxIterations;
  ...
  /// \brief The number of times the linear solver may fail before the
  /// optimisation is aborted. (>0 only if a fallback is available!)
...
inline std::ostream& operator<<(
    std::ostream& out, const aslam::backend::Optimizer2Options& options) {
  /// \brief stop when steps cause changes in the objective function below this
  /// threshold.
  ...
  /// \brief what value of lambda (for Levenberg-Marquardt) should be used when
  /// initializing the optimization
  /// \brief should we print out some information each iteration?
  out << "\tverbose: " << options.verbose << std::endl;
  /// \brief The number of times the linear solver may fail before the
  /// optimisation is aborted. (>0 only if a fallback is available!)
  out << "\tlinearSolverMaximumFails: " << options.linearSolverMaximumFails
      << std::endl;
  /// \brief The number of threads to use
