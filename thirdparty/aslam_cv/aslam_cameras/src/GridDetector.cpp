...
GridDetector::GridDetector(std::shared_ptr<CameraGeometryBase> geometry,
...
void GridDetector::initCameraGeometry(
...
bool GridDetector::initCameraGeometryFromObservation(const cv::Mat &image) {
  std::shared_ptr<std::vector<cv::Mat>> images_ptr =
      std::make_shared<std::vector<cv::Mat>>();
  images_ptr->push_back(image);

  return initCameraGeometryFromObservations(images_ptr);
}

bool GridDetector::initCameraGeometryFromObservations(
    std::shared_ptr<std::vector<cv::Mat>> images_ptr) {
...
bool GridDetector::findTarget(
  ...
  auto compute_stats = [&](double &mean, double &std,
    ...
    unsigned int numCorners =
        outObservation.getCornersImageFrame(corners_detected);
    ...
    for (unsigned int i = 0; i < numCorners; i++) {
      ...
      reprojection_errors_norm(i, 0) =
          sqrt(reprojection_err.x * reprojection_err.x +
    ...
    for (unsigned int i = 0; i < corners_detected.size(); i++) {
      if (reprojection_errors_norm(i, 0) >
        ...
        SM_DEBUG_STREAM("removed target point with reprojection error of "
                            << reprojection_errors_norm(i, 0) << " (mean: "
                            << mean << ", std: " << std << ")\n";);
      }
    }
