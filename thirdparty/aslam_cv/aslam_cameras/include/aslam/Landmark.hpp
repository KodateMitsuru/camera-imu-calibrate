...
class Landmark {
 public:
  ...
  /// get the Descriptor for the Landmark.
  std::shared_ptr<DescriptorBase> descriptorPtr();
  ...
  void setDescriptorPtr(std::shared_ptr<DescriptorBase> descriptor);
  ...
  /// \brief get the UncertainHomogeneousPoint, relative to the associated frame
  /// (see frameId())
  const sm::kinematics::UncertainHomogeneousPoint& point() const;

  /// \brief set the UncertainHomogeneousPoint, relative to the associated frame
  /// (see frameId())
  ...
  /// \brief Support for unit testing. Is this landmark binary equal to the
  /// other?
 ...
 private:
  ...
  /// \brief the UncertainHomogeneousPoint, expressed relative to the frame
  /// (frameId())
