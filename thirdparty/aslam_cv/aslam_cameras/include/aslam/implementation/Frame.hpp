...
template <typename C>
std::shared_ptr<C> Frame<C>::geometryPtr() const {
  return _geometry;
}
...
void Frame<C>::setGeometryBase(
  ...
  std::shared_ptr<C> cam = std::dynamic_pointer_cast<C>(geometry);
...
void Frame<C>::setGeometry(
    const std::shared_ptr<typename Frame<C>::camera_geometry_t>
        &cameraGeometryPtr) {
...
// The keypoint time. This will be something like: frame.time() +
...
// Keypoints can only be added, never removed. This is because data associations
// between frames are stored as indices.
template <typename C>
...
template <typename C>
std::shared_ptr<cameras::CameraGeometryBase> Frame<C>::geometryBase() {
  return _geometry;
}

template <typename C>
std::shared_ptr<const cameras::CameraGeometryBase> Frame<C>::geometryBase()
    const {
  return _geometry;
}
...
bool Frame<C>::computeReprojectionErrorUhp(
    ...
    double &outReprojectionError) const {
  return computeReprojectionErrorUhp(kid.keypointIndex, p,
...
void Frame<C>::load(Archive &ar, const unsigned int version) {
  ...
  // boost::serialization::void_cast_register< aslam::Frame< C >,
  // aslam::FrameBase>(static_cast<aslam::Frame< C > *>(NULL), static_cast<
  // FrameBase * >(NULL) );
  if (version >= 1) {
    ar >> BOOST_SERIALIZATION_BASE_OBJECT_NVP(FrameBase);
  }
