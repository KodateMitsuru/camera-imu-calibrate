...
class Frame : public FrameBase {
 public:
  ...
  typedef std::vector<keypoint_t, Eigen::aligned_allocator<keypoint_t> >
      keypoint_list_t;
  ...
  std::shared_ptr<camera_geometry_t> geometryPtr() const;

  /// \brief set the camera geometry
  void setGeometry(const std::shared_ptr<camera_geometry_t>& cameraGeometryPtr);

  /// \brief get the base camera geometry
  virtual std::shared_ptr<cameras::CameraGeometryBase> geometryBase();

  /// \brief get the base camera geometry
  virtual std::shared_ptr<const cameras::CameraGeometryBase> geometryBase()
      const;

  /// \brief this will throw if the dynamic cast to the underlying type fails.
  virtual void setGeometryBase(
      const std::shared_ptr<cameras::CameraGeometryBase>& geometry);

  /// \brief this will clear the camera geometry and make this frame object not
  /// that useful.
  ...
  /// \brief Keypoints can only be added, never removed. This is because data
  ...
  /// \brief compute the reprojection error for this keypoint from a Euclidean
  /// point
  ...
  /// \brief compute the reprojection error for this keypoint from a Homogeneous
  /// point
  ///        returns true if the reprojection was successful
  virtual bool computeReprojectionError4(size_t i, const Eigen::Vector4d& p,
                                         double& outReprojectionError) const;

  /// \brief compute the reprojection error for this keypoint from an uncertain
  /// Homogeneous point
  ///        returns true if the reprojection was successful
  ...
  /// \brief The keypoint time. This will be something like: frame.time() +
  /// geometry.temporalOffset( keypoint(i) );
  ...
  //         virtual bool getLandmark(size_t k, HomogeneousPoint & outLandmark)
  //         const = 0; virtual bool getLandmark(const KeypointIdentifier & k,
  //         HomogeneousPoint & outLandmark) const = 0; virtual bool
  //         getLandmark(const KeypointIdentifier & k, UncertainHomogeneousPoint
  //         & outLandmark) const = 0; virtual bool getLandmark(size_t k,
  //         UncertainHomogeneousPoint & outLandmark) const = 0;
  ...
  /// \brief the base time for this frame. i.e. the time at which integration
  /// started.
  ...
  virtual std::shared_ptr<const aslam::Image> getOctaves() const {
    return _image;
  }

  virtual std::shared_ptr<aslam::Image> getOctavesMutable() { return _image; }
   ...
   *
   * @param kp An array of keypoints of size N. They are assumed to be all of
   * the same underlying keypoint type.
 ...
 private:
  ...
  std::shared_ptr<aslam::Image> _image;
  ...
  std::shared_ptr<camera_geometry_t> _geometry;
