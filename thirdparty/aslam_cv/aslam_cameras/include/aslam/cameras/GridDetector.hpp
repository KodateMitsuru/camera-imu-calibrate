...
class GridDetector {
 public:
  ...
  struct GridDetectorOptions {
    ...
    /// \brief filter corner outliers that have a reprojection error above a
    /// certain threshold
    bool filterCornerOutliers;

    /// \brief a corner is flagged as inactive, if the its reprojection error is
    /// greater than
    ///        mean + filterCornerSigmaThreshold * std  (statistics over all
    ///        corners on board)
    float filterCornerSigmaThreshold;

    /// \brief filter corner outliers: filtering is only active above this
  ...
  GridDetector(
      std::shared_ptr<CameraGeometryBase> geometry,
  ...
  void initCameraGeometry(std::shared_ptr<CameraGeometryBase> geometry);
  ...
  /// \return true if successful
  bool initCameraGeometryFromObservations(
      std::shared_ptr<std::vector<cv::Mat> > images_ptr);

  /// \brief get the underlying geometry
  std::shared_ptr<CameraGeometryBase> geometry() const { return _geometry; };
  ...
  ///        This method will also estimate and fill in the transformation of
  ///        the camera with respect to the grid.
  bool findTarget(const cv::Mat &image, const aslam::Time &stamp,
                  GridCalibrationTargetObservation &outObservation) const;

  bool findTarget(const cv::Mat &image,
                  GridCalibrationTargetObservation &outObservation) const;

  /// \brief Find the target but don't estimate the transformation.
  bool findTargetNoTransformation(
      const cv::Mat &image, const aslam::Time &stamp,
      GridCalibrationTargetObservation &outObservation) const;
 ...
 private:
  /// the camera geometry
  std::shared_ptr<CameraGeometryBase> _geometry;
