...
BoostPropertyTree::BoostPropertyTree(const std::string& baseNamespace)
    : PropertyTree(std::shared_ptr<PropertyTreeImplementation>(
                       new BoostPropertyTreeImplementation),
                   baseNamespace) {}

BoostPropertyTree::BoostPropertyTree(const boost::property_tree::ptree& ptree,
                                     const std::string& baseNamespace)
    : PropertyTree(std::shared_ptr<PropertyTreeImplementation>(
                       new BoostPropertyTreeImplementation(ptree)),
                   baseNamespace) {}
...
BoostPropertyTree::const_iterator BoostPropertyTree::begin() const {
  return dynamic_cast<const BoostPropertyTreeImplementation*>(_imp.get())
      ->begin();
}
