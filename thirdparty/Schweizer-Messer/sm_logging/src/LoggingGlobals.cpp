 ...
 *
 * This is used for the case where a logger's verbosity level changes, and we
 * need to reset the enabled status of all the logging statements.
...
void LoggingGlobals::checkLogLocationEnabledNoLock(LogLocation* loc) {
  loc->_loggerEnabled =
      loc->_level >= _level &&
      (loc->_streamName.size() == 0 || isNamedStreamEnabled(loc->_streamName));
}
 ...
 *
 * This must be called if a log4cxx::Logger's level has been changed in the
 * middle of an application run. Because of the way the static guard for
 * enablement works, if a logger's level is changed and this function is not
 * called, only logging statements which are first hit *after* the change will
...
#ifdef _MSC_VER
  size_t total = vsnprintf_s(&_globalCharBuffer[0], _globalCharBuffer.size(),
                             _globalCharBuffer.size(), fmt, args);
#else
  size_t total =
      vsnprintf(&_globalCharBuffer[0], _globalCharBuffer.size(), fmt, args);
#endif
  if (total >= _globalCharBuffer.size()) {
    _globalCharBuffer.resize(total + 2);
#ifdef _MSC_VER
    total = vsnprintf_s(&_globalCharBuffer[0], _globalCharBuffer.size(),
                        _globalCharBuffer.size(), fmt, arg_copy);
#else
    total = vsnprintf(&_globalCharBuffer[0], _globalCharBuffer.size(), fmt,
                      arg_copy);
#endif
  }
  va_end(arg_copy);
...
void LoggingGlobals::print(const char* streamName, Level level,
  ...
  try {
    _logger->log(LoggingEvent(streamName, level, file, line, function,
                              &_globalCharBuffer[0],
...
void LoggingGlobals::print(const char* streamName, Level level,
                           vectorstream& ss, const char* file, int line,
                           const char* function) {
  if (_shutting_down) return;
...
void LoggingGlobals::setLogger(std::shared_ptr<Logger> logger) {
  if (logger.get()) {
    _logger = logger;
  }
}
std::shared_ptr<Logger> LoggingGlobals::getLogger() { return _logger; }
...
std::shared_ptr<Logger> getLogger() { return g_logging_globals.getLogger(); }
